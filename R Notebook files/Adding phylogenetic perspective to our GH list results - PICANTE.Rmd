...
Picante should come loaded with base R, but it it doesn't it can be installed routinely (from CRAN repository)
...
#Here, i= 6 because there are six samples in six rows of the commuinty matrix.
...
Alsthough the "read.tree" command in APE can create a phylo object without tree lengths,  such a tree will not work with PICANTE. Lengths need to be part of the phylogeny (and the R phylogeny object that is created using the "read.tree" command. If lengths are unavailable, they can all be entered as 1.
...
Be sure not so skip over this important step with comments!

```{r}
#Also, this read as a dataframe, but for graphing (below) it needs to be a matrix
...
#This first line sets up a grid for mapping multiple phylogenies on one panel
#This is a general command that could be used almost anywhere
#We'll used it avoe to set up a 2,3 grid. Here, instead, we set us a 2,2 grid for four plots.
par(mfrow = c(2, 2))
...
Visually, it's clear that sample "five"" is quite different as compared to "one" and "three" and "two" which have taxa confined to just one sub-clade within the tree. In contrast, "five" has taxa from both sub-clades in the overall tree. Above, we looked at the metric known as Faith's phylogenetic diversity (PD) and wil now do that for our new sample:
...
Nowlet's plot the size traits and binary/discrete traits (either have or do not have a trait).
...
Parallel to that example, you have compiled a lot of different lists. 

This includes the list of the "small" and "medium" and "large" (overall) 


I recommend processing these three lists in a way similar to the individual greenhouse lists. First they need to be turned into the format with "ListID" and "SpeciesID" and "frequency" (which will be 1 or 0), then use rbind to combine them all. Then, to get them into a presence/absence matrix, use the LABDSV package, the matrify function. The goal is to do something similar to what is on lines 172-182 above.


You then want to plot showing all the taxa, on one whole tree, and separate trees for each list showing dots for which species are present. I think you can do that fairly easy with the new V.PhyloMaker package (see the other notebook for a slightly different example with street trees, but similar enough that I think it can provide valuable guidance. The R notebook file is called  Phylogenies - Using Street Tree Data). 

After you do that, another way to do this might be to create a dataframe (in R or in Excel) that is somewhat similar to the trait one (imported above, line 231). That dataframe will have the complete list of taxa in the first column and could then also have a "discrete character" code in the second column, which could code corresponding to the category for each species (e.g., S M L to code for exclusive to small, med or large; SM SL ML to indicate  two-way overlap among those three categories; SML to indicate three-way overlab - these can basically be the List ID variable in the original lists that you can then combine using the rbind function ... it's not really a "trait" but it is an interesting attribute of each taxon)

Yet another analysis to do, going back to the one giant list of all the greenhouses combined, which had a frequency column if I remember correctly. Possibly the frequency of each species could be mapped as a "trait" using color dots, with a heat map type of scale? But for that, it might make more sense to install and load/learn the PHYTOOLS package. See this link
...
Another hard part is going to be figuring out how to visualize this meaningfully and appealingly. Let's first see if we can get the analyses done! (Also, phytools is a good package for making visualizations, so that's another incentive for me and Jared and you to learn to use it)
